...
public class ServiceBuilder {
    ...
    private <P extends ParameterBase> ServiceBuilder withCreator(boolean rootCreator,
        ...
        Specification
                specification = this.specification.computeIfAbsent(createdClass, s -> new Specification());
        ...
        specification.setDefaultParameter(defaultParameter == null ? LaunchType.AUTOMATIC : defaultParameter);
    ...
    public Service build() {
        ...
        InstanceCreator instanceCreator =
                new InstanceCreator(specification, this::getInstanceKey);
        ...
        Map<Class<?>, Specification> instancesToInitiate = rootSpecification.isEmpty() ? specification : rootSpecification;
        ...
        for (Map.Entry<String, Specification> entry : instanceCreator.getDynamicSpecification().entrySet()) {
    ...
    private void assignDefaultCreators(Map<Class<?>, Specification> instances) {
        for (Specification specification : instances.values()) {
            for (Creator<?, ?> defaultCreator : specification.getCreator().defaultCreators()) {
                ...
                Specification createdSpecification = instances.computeIfAbsent(createdClass, s -> new Specification());

                //TODO: jeśli default creatory są te same, to nie jest to problem
                if (createdSpecification.getDefaultCreator() != null && createdSpecification.getCreator() == null) {
                    throw new IllegalStateException(
                            format("Found duplicated default creators (c1: %s, c2: %s)" +
                                   ...
                                   createdSpecification.getDefaultCreator().getClass().getSimpleName(),
